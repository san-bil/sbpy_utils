function [Xwh, mu, invMat, whMat] = whiten(X,spec_gap_thresh, epsilon)
...
svp = max(1, length(find(sigma/(sigma(1)) > spec_gap_thresh)));

tmp=diag(sigma);
trunc_sigma = tmp(1:svp);

whMat = sqrt(size(X,1)-1)*U(:,1:svp)*sqrtm(inv(diag(trunc_sigma) + eye(max(size(trunc_sigma)))*epsilon))*U(:,1:svp)';
Xwh = X*whMat;  
invMat = pinv(whMat);
